...
function AuthContentGlassy() {
  ...
  const handleRegLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                <AnimatePresence mode="wait">
                  ...
                  {activeTab === 'register' && (
                    <motion.form key="register" variants={containerVars} initial="hidden" animate="show" exit="hidden" onSubmit={handleRegister}>
                      ...
                      <motion.div variants={itemVars}><GlassInput id="rWebsite" type="url" label="Studio Website Link (Optional)" required={false} value={regWebsite} onChange={(e:any)=>setRegWebsite(e.target.value)} /></motion.div>
                      
                      {/* Logo Uploader */}
                      <motion.div variants={itemVars} className="relative mb-6 z-10 pt-2">
                        <div className="border border-dashed border-slate-350 rounded-[12px] p-4 bg-white/50 hover:bg-white flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ease-out">
                          <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" onChange={handleRegLogoUpload} />
                          <Camera className="w-5 h-5 text-slate-400 mb-1" />
                          <span className="text-xs font-bold text-slate-600">{regLogo ? 'Change Studio Logo (Optional)' : 'Upload Studio Logo (Optional)'}</span>
                          {regLogo && <span className="text-[10px] text-emerald-500 mt-1 font-bold">Logo Loaded!</span>}
                        </div>
                      </motion.div>
