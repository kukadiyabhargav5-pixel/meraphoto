...
export default function AdminQueriesPage() {
  const [contacts, setContacts] = useState<SupportTicket[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedContact, setSelectedContact] = useState<SupportTicket | null>(null);
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const res = await apiClient.put(`/support/ticket/${id}/status`, { status });
      if (res.data.message) {
  ...
  const handleSendReply = async () => {
    ...
    try {
      const res = await apiClient.post(`/support/ticket/${selectedContact._id}/reply`, {
        message: replyText
      });
      if (res.data.message) {
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
              ...
              <span className="inline-flex items-center justify-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-[#c5a880]/10 text-[#a07c4c] border border-[#c5a880]/20">
                {contacts.filter(c => c.status === 'OPEN').length} New
              </span>
            </h1>
            <p className="text-sm font-bold text-slate-400 mt-2 uppercase tracking-[0.15em]">Manage studio support tickets & inquiries</p>
          </div>
              ...
              <AnimatePresence mode="popLayout">
                {loading ? (
                  ...
                  filteredContacts.map((contact, index) => {
                    ...
                    const isNew = contact.status === 'OPEN';
                    ...
                    return (
                      <motion.button
                        ...
                        onClick={() => {
                          ...
                          setIsReplying(false);
                          setReplyText('');
                          if (contact.status === 'OPEN') updateStatus(contact._id, 'IN_PROGRESS');
                        }}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2">
                              <p className={`text-xs truncate flex-1 ${isNew ? 'text-slate-700 font-medium' : 'text-slate-400'}`}>
                                {contact.messages && contact.messages.length > 0 ? contact.messages[0].message : contact.subject}
                              </p>
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 bg-white/40 flex flex-col gap-8">
                    ...
                    <div>
                      ...
                      <div className="p-6 bg-slate-50 border border-slate-100 rounded-2xl text-slate-700 whitespace-pre-wrap leading-relaxed text-sm shadow-inner relative">
                        ...
                        <span className="relative z-10 font-bold mb-2 block border-b pb-2">Subject: {selectedContact.subject}</span>
                        <div className="relative z-10 flex flex-col gap-6">
                          {selectedContact.messages.map((msg, i) => (
                            <div key={i} className={`flex flex-col ${msg.sender === 'ADMIN' ? 'items-end' : 'items-start'}`}>
                              <span className="text-[10px] font-bold text-slate-400 mb-1">{msg.sender} - {new Date(msg.timestamp).toLocaleString()}</span>
                              <div className={`p-4 rounded-xl max-w-[85%] ${msg.sender === 'ADMIN' ? 'bg-[#c5a880]/10 text-slate-800' : 'bg-white border border-slate-200 text-slate-600'}`}>
                                {msg.message}
                                {msg.attachments && msg.attachments.length > 0 && (
                                  <div className="mt-3 pt-3 border-t border-slate-200/50 flex flex-wrap gap-2">
                                    {msg.attachments.map((att: string, attIdx: number) => {
                                      const isVideo = att.toLowerCase().match(/\.(mp4|webm|ogg)$/);
                                      return (
                                        <a key={attIdx} href={att} target="_blank" rel="noopener noreferrer" className="block w-20 h-20 rounded-md overflow-hidden border border-slate-200 hover:border-[#c5a880] transition-colors relative group">
                                          {isVideo ? (
                                            <div className="w-full h-full bg-slate-200 flex items-center justify-center">
                                              <div className="w-6 h-6 rounded-full bg-black/50 flex items-center justify-center text-white text-[10px]">▶</div>
                                            </div>
                                          ) : (
